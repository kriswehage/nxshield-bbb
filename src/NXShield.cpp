...
/*! @brief Resets all encoder values and motor parameters.  Leaves PIDs
untouched.
*/
...
@param which_motors 1, 2, or SH_Motor_Both
@param speed in range [-100, 100]
@param duration in seconds
@param bit flags for control purposes
*/
bool NXShieldBank::motorSetSpeedTimeAndControl(
  ...
  if(which_motors == SH_Motor_Both) {
    /* Clear the 'go right now' flag */
    control &= ~SH_CONTROL_GO;
...
/*! @brief set encoder speed, time and control in buffer
...
/*! @brief Take a speed and direction and convert to just a speed
*/
...
/*! @brief run motor unlimited
@param  which_motors 1, 2, or Both
...
@param speed [-100, +100]
@param duration in seconds
@param wait_for_completion Wait_For or Dont_Wait
@param next_action Brake, BrakeHold or Float
*/
...
/*! @brief runs the motor(s) the specified number of rotations
@param which_motors 1, 2, or Both
@param direction Forward or Reverse
@param speed [-100, 100]
@param rotations number of full rotations of the motor
@param wait_for_completion Completion_ Wait_For or Dont_Wait
@param next_action Next_Action_ Brake, BrakeHold or Float
*/
...
/*! @brief The stop command will only stop the motor(s) by making them float/coast
or brake.  Even if you specify Next_Action_BrakeHold, the motor
will only brake, not hold.
*/
bool NXShieldBank::motorStop(
  SH_Motor which_motors,
  SH_Next_Action next_action) {
...
//   };
//   if(red >= 0 && red <= 8) {
//     redLED = intensity[red];
//   }
//   if(green >= 0 && green <= 8) {
//     greenLED = intensity[green];
//   }
//   if(blue >= 0 && blue <= 8) {
//     blueLED = intensity[blue];
//   }
// }
//
...
//     return btnState_left;
//   } else if (btn==BTN_RIGHT) {
//     return btnState_right;
//   }
// }
//
